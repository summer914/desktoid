 ...
 * Date: $Date: 2007-05-11 04:49:09 -0700 (Fri, 11 May 2007) $
 *
...
public class RdpPacket_Localised extends RdpPacket {
	...
	public int getBigEndian32(int where) {
		return ((bb[where] & 0xff) << 24) + ((bb[where + 1] & 0xff) << 16)
				+ ((bb[where + 2] & 0xff) << 8) + (bb[where + 3] & 0xff);
	}

	public int getBigEndian32() {
		return ((bb[position++] & 0xff) << 24)
				+ ((bb[position++] & 0xff) << 16)
				+ ((bb[position++] & 0xff) << 8) + (bb[position++] & 0xff);
	}
